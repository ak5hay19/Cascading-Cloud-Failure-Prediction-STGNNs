...
def build_raw_failures(df):
    """
    Build raw failure events — actual (machine, window) failure occurrences.

    Unlike build_labels() which shifts failures BACKWARD to create predictive
    labels ("will fail within K windows"), this stores WHERE failures actually
    happen. Used for cascade modeling: at training time, cascade targets for
    input window w are raw_failures[w+1], raw_failures[w+2], raw_failures[w+3].
    """
    print("Building raw failure events for cascade modeling...")
    if "failed" not in df.columns:
        if "instance_events_type" in df.columns:
            df["failed"] = df["instance_events_type"].isin([5, 7, 8]).astype(int)
        else:
            print("  ERROR: Cannot determine failures!")
            return pd.DataFrame(columns=["machine_id", "time_window"])
    ...
    print(f"  {len(raw):,} raw failure events across "
          f"{raw['time_window'].nunique()} windows")
    return raw
...
def main():
    ...
    raw_failures = build_raw_failures(df)
    ...
    raw_failures.to_parquet("processed/raw_failures.parquet", index=False)
    ...
    print(f"  Raw failure events: {len(raw_failures):,}")
    print(f"\n  Saved to ./processed/")
